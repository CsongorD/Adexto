    ...
    <div className="overflow-hidden rounded-lg border border-primary-100 bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-lg sm:rounded-xl">
      <div className="relative">
        ...
        <div className="absolute left-2 top-2 rounded-full bg-white/90 px-2 py-1 backdrop-blur-sm sm:left-3 sm:top-3 sm:px-3">
          <h3 className="text-xs font-semibold text-primary-800 sm:text-sm">
            {name}
          </h3>
        </div>
      </div>
      ...
      <div className="p-3 sm:p-4">
        <h4 className="mb-2 text-center text-xs font-medium text-primary-600 sm:mb-3 sm:text-sm">
          Dostupne dužine
        </h4>
        ...
        <div className="space-y-1 sm:space-y-2">
          {options.map((option, index) => (
            <div
              key={index}
              className="flex items-center justify-between rounded-md bg-primary-100 px-2 py-1.5 sm:rounded-lg sm:px-3 sm:py-2"
            >
